...
INSTALLATION
------------
- unzip JAP_WS_REMOTE_NODE_OPENSHIFT-X.X.X.zip
- open JAP_WS_REMOTE_NODE_OPENSHIFT-X.X.X
- configure JAP_WS_REMOTE.json
- open CONSOLE
	- WINDOWS
		- rhc app create -a application-id -t nodejs-0.6
		- cd application-id
		- xcopy ..\JAP_WS_REMOTE_NODE_OPENSHIFT-X.X.X\*.* /s /y
		- git add -A
		- git commit -m "JAP"
		- git push
	- LINUX
		- rhc app create -a application-id -t nodejs-0.6
		- cd application-id
		- cp -r ../JAP_WS_REMOTE_NODE_OPENSHIFT-X.X.X/. .
